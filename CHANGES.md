...
- 2026-09-17: Rule schema 0.7 adds rule-selected input preparation. Tool-specific joins live in `parserV2/joins/alphadia.py` and `maxquant.py`; parent composition reads inputs and shares one prepared frame across detection and level parsing. AlphaDIA 1.12 enriches authoritative matrix quantities with precursor metadata. MaxQuant replaces independent-table rules with one long-table rule: wide measurements are unpivoted, evidence foreign keys are joined with experiment identity, and join fan-out does not multiply ion measurements. `apb2 convert --companion PATH` accepts additional files; directories and explicit `InputFiles` bindings also work. Preparation provenance records sources, elapsed time, row count and estimated frame size. Generated schemas, tests and packaged documents use 0.7 without a compatibility path. DIA-NN is unchanged.
